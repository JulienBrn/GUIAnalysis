...
class ParsedHumanSTNMUADataBase(GUIDataFrame):
    ...
    def compute_df(self, db):
        raw_df =  db.copy()
        raw_df["file_keys"] = raw_df.apply(lambda row: ["CElectrode{}".format(row["Electrode"])], axis=1) 
        raw_df["signal_path"] = raw_df.apply(lambda row: toolbox.DataPath(row["file_path"], row["file_keys"]), axis=1)
        raw_df["signal_fs_path"] = raw_df.apply(lambda row: toolbox.DataPath(row["file_path"], ["CElectrode{}_KHz".format(row["Electrode"])]), axis=1)
        ...
        raw_df["signal"] = raw_df.apply(lambda row: self.computation_m.declare_computable_ressource(declare_raw_sig, {"dp": row["signal_path"]}, toolbox.np_loader, "human_input_mua", True), axis=1)
        raw_df["signal_fs"] = raw_df.apply(lambda row: self.computation_m.declare_computable_ressource(declare_raw_fs, {"dp": row["signal_fs_path"]}, toolbox.float_loader, "human_input_fs", True), axis=1)
